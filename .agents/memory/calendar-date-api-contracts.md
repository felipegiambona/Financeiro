...
Date-only financial values such as transaction dates, due dates, invoice dates, and card-history dates must remain `YYYY-MM-DD` at API boundaries. Preserve raw date-only request strings before generated schemas coerce them to UTC midnight; if a schema coerces a response to `Date`, validate it at São Paulo noon and serialize it back to a São Paulo calendar-date key before sending JSON.
